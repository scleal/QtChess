...
void MainWindow::DrawStatus(QPainter& painter){
    ...
    if (noValidMoves && inCheck){
        painter.drawText(200, 280, curPlayer.append(" IN CHECKMATE"));
        QString winner = currentPlayer == WHITE ? "BLACK" : "WHITE";
        painter.drawText(200, 260, winner.append(" WINS"));
    } else if (noValidMoves){
        painter.drawText(200, 280, "STALEMATE");
    } else if (inCheck){
        painter.drawText(200, 280, curPlayer.append(" IN CHECK"));
    } else if (promotion){
        painter.drawText(200, 280, "Queen  (Q)");
        painter.drawText(200, 260, "Rook   (R)");
        painter.drawText(200, 240, "Bishop (B)");
        painter.drawText(200, 220, "Knight (K)");
    }
}
